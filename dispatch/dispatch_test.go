...
func TestAsync(t *testing.T) {
	ok := make(chan bool)
	player := avcore.AVPlayer_playerWithURL_(core.NSURL_fileURLWithPath_isDirectory_(core.String("/Users/anhoder/Desktop/1.mp3"), false))
	Async(MainQueue(), func() {
		...
		go func() {
			for {
				...
				player.SeekToTime_(core.CMTime{Value: 1000000000 + cur.Value, Timescale: cur.Timescale, Flags: cur.Flags})
				fmt.Println(player.CurrentItem().Duration(), player.CurrentTime())
			}
		}()
